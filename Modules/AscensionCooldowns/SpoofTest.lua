...
local SpoofTest = ACD:NewModule("SpoofTest")

local PAGE_SIZE = 9

local function sortedRegistryIDs()
    local ids = {}
    for spellID, entry in pairs(ACD.Registry.entries) do
        if entry.valid then table.insert(ids, spellID) end
    end
    table.sort(ids, function(left, right)
        local leftName = string.lower(ACD.SpellInfo:ResolveSpellName(left))
        local rightName = string.lower(ACD.SpellInfo:ResolveSpellName(right))
...
function SpoofTest:GetDuration()
    ...
    duration = math.max(3, math.min(duration, ACD.Constants.MAX_COOLDOWN))
    ACD.db.profile.spoofUI.duration = duration
...
function SpoofTest:Use(spellID)
    ...
    local player = ACD.State:GetOrCreate(playerKey, identity, "SELF")
    player.source = "SELF"
    player.lastSeen = now
    ...
    ACD.State:Changed("spoof cast")
    if ACD.Comms.initialized then
        ACD.Comms:SendCast(spellID, { remaining = duration, duration = duration }, nil)
        ACD.Comms:ScheduleState(0.2, "spoof cast")
    end
    ACD:Print("SpoofTest used " .. ACD.SpellInfo:ResolveSpellName(spellID)
        .. " (" .. tostring(spellID) .. ") for " .. tostring(duration) .. " sec")
end

function SpoofTest:Reset()
    ACD.Spellbook:Scan("spoof reset")
    if ACD.Comms.initialized then ACD.Comms:SendState(true, "spoof reset") end
    ACD:Print("SpoofTest cleared; real spell state restored")
end
...
function SpoofTest:Refresh()
    ...
    for index = 1, PAGE_SIZE do
        ...
        if spellID then
            row.spellID = spellID
            row.icon:SetTexture(ACD.SpellInfo:ResolveSpellIcon(spellID))
            row.label:SetText(ACD.SpellInfo:ResolveSpellName(spellID) .. "  |cff888888" .. spellID .. "|r")
...
function SpoofTest:Initialize()
    self.rows = {}
    self.page = 1
    local profile = ACD.db.profile.spoofUI
    local frame = CreateFrame("Frame", "ActuallyACDSpoofTestFrame", UIParent)
    frame:SetWidth(355)
    frame:SetHeight(379)
    ...
    frame.title:SetText("ACD SpoofTest")
