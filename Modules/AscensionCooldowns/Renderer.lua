...
local Renderer = ACD:NewModule("Renderer")
...
function Renderer:GetSourcePlayers()
    if ACD.TestMode and ACD.TestMode.active then return ACD.TestMode.players end
    return ACD.State:GetPlayers()
end

function Renderer:ShouldDisplay(playerKey, spellID, spell, now)
    local profile = ACD.db and ACD.db.profile
    if not profile or not profile.enabled then return false end
    if profile.hideWhileSolo and not ACD.Roster:IsGrouped() then return false end
    local selfKey = ACD.Roster:GetPlayer()
...
function Renderer:BuildRow(playerKey, player, spellID, spell, now)
    local entry = ACD.Registry:Get(spellID)
    local spellSettings = ACD.db.profile.spells[spellID] or {}
    ...
    return {
        ...
        spellID = spellID,
        spellName = ACD.SpellInfo:ResolveSpellName(spellID),
        icon = ACD.SpellInfo:ResolveSpellIcon(spellID),
...
function Renderer:OnUpdate(elapsed)
    ...
    if self.elapsed < ACD.Constants.UI_UPDATE_INTERVAL then return end
    self.elapsed = 0
    local now = ACD:Now()
    ...
    if ACD.State:ExpireRemoteReports() and ACD.Comms.initialized then
