...
local State = ACD:NewModule("State")
...
function State:Changed(reason)
    ...
    if ACD.Renderer and ACD.Renderer.MarkDirty then
        ACD.Renderer:MarkDirty(reason)
    end
end
...
function State:ApplyReport(playerKey, identity, session, sequence, capabilityRevision, rows)
    ...
    peer.protocol = ACD.Constants.PROTOCOL_VERSION
...
function State:ExpireRemoteReports()
    ...
    for playerKey, player in pairs(self.players) do
        if player.source == "REPORT" then
            ...
            if age >= ACD.Constants.REPORT_STALE_AFTER and not player.stale then
                player.stale = true
                requested = true
                self:Changed("stale report")
            end
            if age >= ACD.Constants.REPORT_FORGET_AFTER then
                local active = false
