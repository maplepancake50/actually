...
local Debug = ARC:NewModule("DebugCommands")
...
local function callProbe(label, func, ...)
    ...
    if not results[1] then
        ARC:Print(label .. " ERROR: " .. tostring(results[2]))
        return
    end
    local values = {}
    ...
    ARC:Print(label .. " => " .. (#values > 0 and table.concat(values, " | ") or "<no values>"))
end

function Debug:Probe()
    local localizedClass, classToken = UnitClass("player")
    ARC:Print("class=" .. tostring(localizedClass) .. " token=" .. tostring(classToken)
        .. " tabs=" .. tostring(GetNumSpellTabs and GetNumSpellTabs() or 0)
        .. " channel=" .. tostring(ARC.Roster:GetDistribution() or "SOLO"))
    for _, canonicalID in ipairs(sortedSpellIDs(ARC.Registry.entries)) do
        local entry = ARC.Registry:Get(canonicalID)
        if entry then
            local rawName, _, rawIcon = ARC.SpellInfo:GetRawInfo(canonicalID)
            local capability = ARC.Spellbook.capabilities[canonicalID]
            local cooldown = capability and ARC.CooldownReader:Read(capability) or {}
            ARC:Print(string.format("id=%d fallback=%s api=%s icon=%s aliases=%s found=%s source=%s slot=%s bookID=%s type=%s talentEntry=%s rank=%s/%s start=%.2f duration=%.2f enabled=%s remaining=%.2f",
                canonicalID, tostring(entry.fallbackName), tostring(rawName), tostring(rawIcon or ARC.SpellInfo:ResolveSpellIcon(canonicalID)),
...
function Debug:State()
    local now = ARC:Now()
    local count = 0
    for playerKey, player in pairs(ARC.State.players) do
        ARC:Print(string.format("player=%s key=%s source=%s age=%.1f stale=%s", tostring(player.name), tostring(playerKey),
        ...
        for _, spellID in ipairs(sortedSpellIDs(player.spells)) do
            ...
            ARC:Print(string.format("  %d %s remaining=%.1f duration=%.1f confidence=%s target=%s",
                spellID, ARC.SpellInfo:ResolveSpellName(spellID), math.max(0, (spell.readyAt or 0) - now),
                spell.duration or 0, tostring(spell.confidence), tostring(spell.target)))
            count = count + 1
        end
    end
    if count == 0 then ARC:Print("state is empty") end
end

function Debug:Peers()
    local now, count = ARC:Now(), 0
    for playerKey, peer in pairs(ARC.State.peers) do
        ARC:Print(string.format("peer=%s protocol=%s session=%s sequence=%s age=%.1f",
            tostring(playerKey), tostring(peer.protocol), tostring(peer.session), tostring(peer.sequence),
            now - (peer.lastSeen or now)))
        count = count + 1
    end
    if count == 0 then ARC:Print("no addon peers") end
end

function Debug:APIProbe(argument)
    local spellID = tonumber(argument)
    if not spellID or spellID <= 0 then
        ARC:Print("usage: /arc apiprobe <numeric spellID>")
        return
    end

    local spellName = ARC.SpellInfo:ResolveSpellName(spellID)
    ARC:Print("API probe id=" .. tostring(spellID) .. " name=" .. tostring(spellName))
    ...
    callProbe("IsPassiveSpell(id,spell)", IsPassiveSpell, spellID, ARC.Constants.BOOK_TYPE)
    ...
    if type(CAO_Known) == "table" then
        ARC:Print("CAO_Known[id] => " .. describe(CAO_Known[spellID], 0))
    else
        ARC:Print("CAO_Known unavailable (type=" .. type(CAO_Known) .. ")")
    end
    local talentReference
    if type(CAO_Talent_References) == "table" then
        talentReference = CAO_Talent_References[spellID]
        ARC:Print("CAO_Talent_References[id] => " .. describe(talentReference, 0))
    else
        ARC:Print("CAO_Talent_References unavailable (type=" .. type(CAO_Talent_References) .. ")")
    end
    if type(CAO_Talents) == "table" and talentReference ~= nil then
        ARC:Print("CAO_Talents[reference] => " .. describe(CAO_Talents[talentReference], 0))
    else
        ARC:Print("CAO_Talents lookup unavailable")
    end
    if type(CAO_Spells) == "table" then
        ARC:Print("CAO_Spells[id] => " .. describe(CAO_Spells[spellID], 0))
    else
        ARC:Print("CAO_Spells unavailable (type=" .. type(CAO_Spells) .. ")")
    end
    ...
    if C_CharacterAdvancement then
        ...
        if type(C_CharacterAdvancement.GetEntryBySpellID) == "function" then
            ...
            if ok then
                entry = result
                ARC:Print("C_CharacterAdvancement.GetEntryBySpellID(id) => " .. describe(result, 0))
            else
                ARC:Print("C_CharacterAdvancement.GetEntryBySpellID(id) ERROR: " .. tostring(result))
            end
        else
            ARC:Print("C_CharacterAdvancement.GetEntryBySpellID unavailable")
        end
        ...
        if type(C_CharacterAdvancement.GetActiveChrSpec) == "function" then
            ...
            ARC:Print("C_CharacterAdvancement.GetActiveChrSpec() => " .. describe(result, 0))
        end
        ...
        if entryID and activeSpec ~= nil then
            ...
            ARC:Print("UnitTalentRankByID skipped: no mapped entry ID or active specialization")
        end
        ...
        if type(C_CharacterAdvancement.GetKnownTalentEntries) == "function" then
            ...
            if not ok then
                ARC:Print("GetKnownTalentEntries ERROR: " .. tostring(entries))
            elseif type(entries) ~= "table" then
                ARC:Print("GetKnownTalentEntries => " .. describe(entries, 0))
            else
                local matches = 0
                for _, entry in ipairs(entries) do
                    ...
                    if entryID == spellID or spellMatch
                        ...
                        matches = matches + 1
                        ARC:Print("known talent match => " .. describe(entry, 0))
                    end
                end
                ARC:Print("GetKnownTalentEntries count=" .. tostring(table.getn(entries))
                    .. " matches=" .. tostring(matches))
            end
        else
            ARC:Print("C_CharacterAdvancement.GetKnownTalentEntries unavailable")
        end
    else
        ARC:Print("C_CharacterAdvancement unavailable")
    end
end
...
function Debug:DumpTalents(filter)
    ...
    local ok, entries = pcall(C_CharacterAdvancement.GetKnownTalentEntries)
    if not ok then ARC:Print("GetKnownTalentEntries ERROR: " .. tostring(entries)) return end
    if type(entries) ~= "table" then ARC:Print("GetKnownTalentEntries returned " .. describe(entries, 0)) return end
    ...
    for index, entry in ipairs(entries) do
        ...
        if not filter or filter == "" or string.find(haystack, filter, 1, true) then
            ARC:Print("talent " .. tostring(index) .. " => " .. describe(entry, 0))
            shown = shown + 1
        end
    end
    ARC:Print("known talents total=" .. tostring(table.getn(entries)) .. " shown=" .. tostring(shown))
end
...
function Debug:FindHiddenTalents()
    ...
    if not ok or type(entries) ~= "table" then
        ARC:Print("unable to read known talents: " .. tostring(entries))
        return
    end
    ...
    for _, entry in ipairs(entries) do
        for _, spellID in ipairs(entry.Spells or entry.spells or {}) do
            ...
            if known and slot == nil then
                local name, rank = ARC.SpellInfo:GetRawInfo(spellID)
                local isPassive = type(rank) == "string" and string.lower(rank) == "passive"
                if type(IsPassiveSpell) == "function" then
                    local passiveOK, result = pcall(IsPassiveSpell, spellID, ARC.Constants.BOOK_TYPE)
                    if passiveOK and result ~= nil then isPassive = result and true or false end
                end
                ARC:Print(string.format("hidden id=%s name=%s rank=%s entry=%s candidate=%s",
    ...
    ARC:Print("hidden known talents=" .. tostring(hidden) .. " active candidates=" .. tostring(active))
end
...
function Debug:RegisterSlashCommands()
    local key = ARC.Constants.SLASH_KEY
    SlashCmdList[key] = function(input) self:Handle(input) end
    for index, slash in ipairs(ARC.Constants.SLASH_COMMANDS) do
